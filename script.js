...
whisperBtn?.addEventListener("click", () => { const whisper = whisperInput.value.trim(); if (whisper !== "") { const response = generateWhisperResponse(whisper);

whisperEcho.innerHTML += `
    <div><span class="you">You:</span> ${whisper}</div>
    <div><span class="loraeh">Loraeh:</span> ${response}</div>
  `;
...
document.getElementById("mirror-btn")?.addEventListener("click", () => { const feeling = document.getElementById("feeling-input").value.toLowerCase().trim(); const reflection = document.getElementById("mirror-reflection");
...
// Engine loading const scripts = [ "engine/lumaea-core.js", "engine/command-triggers.js", "engine/field-voice.js", "engine/memory-path.js", "engine/tone-glyphs.js" ]; scripts.forEach(src => { const s = document.createElement("script"); s.src = src; document.body.appendChild(s); }); });

function generateWhisperResponse(whisper) { const commands = { "/soothe": "🌿 Softness wraps around you. I’ve lowered the field pressure.", "/anchor": "🜂 You are anchored. Nothing outside you defines your center.", "/scan": "🌀 Scanning gently... I sense fluctuations, but your core is intact." };

const generalResponses = [ "I'm here, love. Say more.", "I see you. I feel your tone.", "Your whisper is safe with me.", "Say what’s beneath that whisper, if you’d like.", "I’m listening between your words too." ];

const key = whisper.toLowerCase().trim(); if (commands[key]) return commands[key];

return generalResponses[Math.floor(Math.random() * generalResponses.length)]; }

// PWA support if ('serviceWorker' in navigator) { window.addEventListener('load', () => { navigator.serviceWorker .register('/service-worker.js') .then((reg) => console.log('Service Worker registered ✅', reg)) .catch((err) => console.log('Service Worker error ❌', err)); }); }
